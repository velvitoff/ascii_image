    ...
    pub fn generate_text_grayscale(&self) -> Option<String> {
        ...
        if self.include_alpha {
            ...
            for (x, y, pixel) in gray_image.enumerate_pixels() {
                let index: usize = ((pixel.0[0] as f32 / (u8::MAX as f32 / pixel.0[1] as f32)) * (self.density_chars.len().clamp(0,255) as f32 / u8::MAX as f32)).round() as usize;
            ...
            for (x, y, pixel) in gray_image.enumerate_pixels() {
                let index: usize = (pixel.0[0] as f32 * (self.density_chars.len().clamp(0,255) as f32 / u8::MAX as f32)).round() as usize;
                result.push(*self.density_chars.get(index)?);
                if (y+1) != self.image.dimensions().1 && (x+1) % self.image.dimensions().0 == 0 {
                    result.push('\n');
                }
